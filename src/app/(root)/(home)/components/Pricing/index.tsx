          ...
          <label className="flex items-center cursor-pointer">
            <div
              className={`relative w-10 h-6 rounded-full transition-colors duration-300 ${
                isAnnual ? "bg-white" : "bg-[#262626]"
              }`}
              onClick={() => setIsAnnual(!isAnnual)}
            >
              <div
                className={`absolute left-1 top-1 w-4 h-4 bg-black rounded-full transition-transform duration-300 ${
        ...
        {plans.map((plan, index) => (
          <div
            ...
            className={`p-6 rounded-xl border-[1.5px] ${
              plan.highlighted ? "border-yellow-400" : "border-gray-700"
            } bg-black`}
          >
